...
- Node.js **22.12+** (모노레포 루트 `package.json` engines 기준 — 앱 단독으로는 20.19+로도 뜨지만 저장소 표준은 22.12+). **Node 22 LTS와 24 LTS**에서 검증되었습니다.
...
저장소를 clone/다운로드한 직후, `node_modules`를 지운 경우, `package.json` 의존성이 바뀐 경우에만 실행합니다. **평소 실행 시에는 필요 없습니다.** (2026-08-28 모노레포 통합 후에는 **저장소 루트의 `npm install` 한 번**이 이 앱 의존성까지 함께 설치합니다 — 아래 명령은 앱 단독 설치 시에만.)
