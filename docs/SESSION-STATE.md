...
3. **M2·M3는 끝났고 M2 게이트도 승인됐다**(아래 표). 다음 행동은 **M4 재판정** — 3단 구조(챗 실사용 11문 + 서버 프로브 회귀 6~9문 + 손상 프로브 2문). 범위·오염 방지 프로토콜은 `MAINTENANCE-PLAN.md`의 "재판정 범위 (M4)" 절이 정본이며, **재판정 시 새로 나타나는 문구 2종을 회귀로 오인하지 말 것**(같은 절의 경고 상자).
