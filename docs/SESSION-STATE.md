...
슬라이스 1 스파이크 판정 통과 → **유지보수 M0~M4 전부 달성**(2026-08-22). **M4 재판정 3단 구조 완주 — 합격선 8/8 달성, 미달·미측정 0건.** 다음은 **M4 게이트 오너 승인** → **M5 Codex 21문** → main 병합 승인.
...
4. 다음 행동은 **M4 게이트 오너 승인 요청** → 승인 시 **M5(Codex 데스크탑 21문)**. M5는 **이사님이 직접** 실행해야 한다(Codex 표면).
5. **재발 방지 기전 설계도 끝나 있다** — 오너 결정 4건 대기(아래 "결정 대기" 절).
...
| # | 결정 | 추천 |
|---|---|---|
| 1 | **A1 선행 수리 3건** — 살해 케이스 2건 추가·`mustNotDetect` 신설·파싱실패 플래그 | 승인 (2~4시간) |
| 2 | 자동 실행 채널 | **저장소 pre-commit만** (`settings.json`은 오너 소유라 미변경) |
| 3 | 8조 제3조 *"중 하나 이상"* → *"임계값·단위와 분모·직전 실적 전부"* | 개정 |
| 4 | 이 설계 자체를 실행표 v1에 돌려 검출력 실측 | (가) v1 대상 |
