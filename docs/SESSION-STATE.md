...
| # | 상태 | 내용 |
|---|---|---|
| ① | **유효** | `measure-latency --fresh`가 **문항 단위**라 같은 문항을 두 번 물으면 중복이 구분되지 않는다(`:71`·`:163-164`) |
| ② | **유효 — 가장 크다** | **부재 역검증기가 저장소에 없다.** M4에서 쓴 `verify-absence-m4.js`는 scratchpad 일회용이다. M5 합격선 4(거짓 부재 0건)의 두 절차 중 하나인데 **존재하지 않는다.** 게다가 정규식이 `role`을 성격 계열로 오분류해 오탐 1건 발생(값이 `"주인공"` = 실제 역할이라 기각) |
| ③ | ~~해소~~ | `refresh-keyword-probes` 파싱 실패 구분 — **2026-08-22 A1-3에서 수리 완료**(`parseFailed` 플래그) |
| ④ | **`[미확인]`으로 강등** | `--since` 파싱 코드는 두 도구가 **동일**하다(`measure-latency.mjs:66` ≡ `refresh-keyword-probes.js:33`). 실제 차이가 있다면 **비교 대상 이벤트**(질문 발화 시각 vs `tool_use` 호출 시각)일 수 있으나 확인하지 않았다 |

**①②는 도구·스크립트만 건드리므로 M5 판정 대상 코드를 흔들지 않는다** — A1에서 실증된 논리. M5 대기 중 처리해도 기준선은 유효하다.
