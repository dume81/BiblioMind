...
슬라이스 1 스파이크 판정 통과 → **유지보수 M0~M4 전부 달성 + M4 게이트 조건부 승인 완료**(2026-08-22). 합격선 8/8, 미달·미측정 0건. 승인 조건 = 소요 퇴행(5.6→6.9초) 원인 규명을 개선 큐 Q1로 등재(**이행 완료**). 다음은 **M5 Codex 21문(이사님 직접)** → main 병합 승인.
...
5. **재발 방지 기전 설계도 끝나 있다** — 오너 결정 **3건 대기**(아래 "결정 대기" 절. A1 선행 수리는 **테스트·도구만 추가**하면 되고 프로덕션 코드 무변경임이 실측됨 → M5 기준선과 충돌하지 않는다).
...
> **한 번에 1건씩만 여쭙는다**(2026-08-22 오너 지시). 아래는 대기열이며 목록 전체를 한 화면에 펼쳐 묻지 않는다.

| # | 결정 | 추천 | 상태 |
|---|---|---|---|
| 1 | **A1 선행 수리 3건** — 살해 케이스 2건 추가·`mustNotDetect` 신설·파싱실패 플래그 | 승인 (2~4시간) | **다음 질의 대상** |
| 2 | 자동 실행 채널 | **저장소 pre-commit만** (`settings.json`은 오너 소유라 미변경) | 대기 |
| 3 | 8조 제3조 *"중 하나 이상"* → *"임계값·단위와 분모·직전 실적 전부"* | 개정 | 대기 |
| ~~4~~ | ~~설계 검정을 실행표 v1 대상으로~~ | — | **철회** — M4가 이미 끝나 v1 실행 기회가 소멸. 검정은 M5 실행표에 붙인다 |

**A1 관련 실측(2026-08-22)**: 살해 입력 2건 + 대조군 1건을 **현재 코드에 직접 실행 → 3/3 통과**. 즉 **프로덕션 코드는 이미 올바르고 테스트만 없었다** — A1 수리는 테스트·도구 추가뿐이므로 M5 판정 대상 코드를 건드리지 않는다. 재현: `scratchpad/kill-cases.mjs`.
