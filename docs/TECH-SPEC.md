...
> 상태: **v2.2 — v2.1(AuraDB 전환·스캐폴딩 패널 반영) 후 총감사(패널 8인) 개정: 검증 계획 6(호출률·부정 대조) 신설, §5.5 404 비치명 처리, §6.4 Aura RTT 재산정, citation 문구 정직화, 과잉 인용 경고, 잠금 감지, README 목차 ⑮~⑰, 부록 A 3건 해소, 신형 콘솔 자격증명 정정 (상세는 부록 C·DECISIONS.md)**  · **v2.3(2026-08-22 유지보수 M2·M3): §6.2.3 질문 원문 앵커 복원 / §6.3 T2 점수 하한 / §6.4 상한 3행 / §4.3-13 인용 밀도 안내 정직화 / §1.14 가정 7행 신설** · **v2.4(2026-08-22): §7.9 하이라이트 상태 진단 스냅샷(`window.__bibliomind`) 신설 — 읽기 전용, 렌더링 무변경** · **v2.5(2026-08-22 슬라이스 3): §1.5 스킵된 페이지의 링크 되살리기 신설 — 재실행 이어받기 공백 해소(네트워크 요청 0회 유지)** · **v2.6(2026-08-22 슬라이스 1.5): §1.4 엔진 플래그 세트 실측 확정 — --max-turns 부재로 제거, --output-schema 구조적 비호환으로 미채택** · **v2.7(2026-08-23 슬라이스 6): §2.4.4 `last_reject_reason` 신설** · **v2.8(2026-08-23 Q6): §2.3.2 정본 엔티티 사전(`canonical_entities`)** · **v2.9(2026-08-23 Q8): §6.3 `nodeFacts`(속성 전달)** · **v2.10(2026-08-26 Q10): §2.3.2 속성 충돌 가시화 + 속성 승자 사전(`property_overrides`) · §2.1.2 사전 필드 등재 · §4.3-8 반환·안내 분기 (4렌즈 반박 패널 반영 — v2.7~v2.9는 인라인만 있고 이 목록에 누락돼 있어 함께 등재)** · **v2.11(2026-08-26 Q7): §2.1.5 시점 규율 지시문 2줄(Event 단일화 — _as_of 접미 미채택) · §2.1.2 수동 지시문 개정 경로 보완(3렌즈 반박 패널 반영)** · **v2.12(2026-08-26 슬라이스 8): §4.3-9 source_remove 확정(원장 전수 처리·키 복원 순서·부분 실패 계약) · §4.3-11 가드·destructive 확정 · §4.3-15 구현 확정(도메인 수집·부분 성공 규정) · §5.1 GET /api/health 신설 · §2.4.4 blocked_at · §2.1.2 "실변경 시 +1" 문면 정정(3렌즈 반박 패널 반영)** · **v2.13(2026-08-27 슬라이스 8.5): §4.3-1·2 구현 확정 — collect_web 상태 판정 규칙(robots 전면 차단 = 실패·중복만 해소 = 부분 성공)·저장 목록 표기(덮어씀 구분), collect_docs 폴더 배치(파일만·이름순·비지원 제외 목록 보고)·차단(blocked) 스킵 정직 안내(force 불가 — source_remove 해제 경로)·절대경로 강제, 양 도구 §4.1 실패 5건 상한 준수(3렌즈 반박 패널 반영)**| 최종 수정: 2026-08-27
...
|---|---|---|
| 명령 골격 | `cmd /c codex exec --skip-git-repo-check -s read-only -C <data/tmp> -o <출력파일> -` (프롬프트는 stdin) | `cmd /c claude -p --output-format json` (프롬프트는 stdin, cwd=`data/tmp`) |
| 결과 수취 | `-o/--output-last-message` 파일에서 최종 메시지 읽기 (stdout 인코딩 이슈 회피). **`--output-schema`는 미채택 확정**(v2.6 — 아래 근거) | stdout의 결과 JSON에서 `result` 필드 추출, `is_error` 필드로 실패 판별 |
| 도구/부작용 차단 | 샌드박스 `read-only` | 헤드리스(`-p`)의 기본 차단으로 충분 — **실측 부작용 0건**(v2.6). `--disallowed-tools`는 실재하나 현 단계 미채택 |
...
- ~~세부 플래그는 2026-08 기준 지식이며, 정확한 플래그 세트는 엔진 스모크에서 확정~~ → **2026-08-22 엔진 스모크로 확정 완료(v2.6, 슬라이스 1.5)**. 두 엔진 각 1회 실호출 성공(codex 노드 6·관계 5 / claude 노드 9·관계 15, 둘 다 스키마 검증 통과). 상세는 `DECISIONS.md`.
  - **정본이 틀렸던 것 2건 — 실측으로 정정**:
    1. **`--max-turns`는 claude CLI에 존재하지 않는다.** 제거했다. 그 플래그 없이도 응답 봉투의 `num_turns`가 **1**로 확인돼 목적이 이미 충족된다.
    2. **`--output-schema`(Codex)는 채택하지 않는다.** OpenAI structured output 스펙이 **모든 object에 `additionalProperties: false`를 요구**해 HTTP 400을 돌려준다(`invalid_json_schema`). 그런데 KG 노드의 `properties`는 **자유 형태 객체**여야 하고, 키를 미리 열거하면 **§2.1의 "도메인 스키마 AI 자동 도출"과 정면 충돌**한다 — 구조적 비호환이다. 없이도 두 엔진 모두 백틱 펜스 없는 순수 JSON을 반환했고, 안전망은 아래 `bad_output` 교정 재호출 1회 규칙이 담당한다.
  - **어댑터 `timeoutMs` 기본값은 codex 기준으로 잡는다** — 동일 프롬프트(2,196자)에서 codex **48.9초** vs claude **15.3초** [실측].
...
- **스킵된 페이지의 링크 되살리기 (v2.5 신설 — 2026-08-22 오너 승인)**: 1차 스킵은 요청을 생략하므로 **그 페이지의 링크도 발견되지 않는다.** 그대로 두면 상한에 걸려 큐에 남은 URL을 **재실행으로 이어받을 수 없고**(BFS가 시작 페이지에서 끝난다), 유일한 회피책이 `--force` 전체 재수집이 되어 무키 저율 호출에서 비용이 크다. ⇒ 스킵 시 **이미 저장된 `Input/<file>.md`를 읽어 그 마크다운에서 링크를 재추출**해 BFS 프런티어에 넣는다. 기준 URL은 프론트매터 `url_normalized`. **네트워크 요청은 여전히 0회**이므로 "요청 자체를 생략"이라는 1차 스킵의 계약은 그대로다. 원장이 `collected`인데 파일이 없으면(사용자가 지웠다) 링크 없이 넘어가고 요약에 1줄 보고한다.
  - 발견 근거: 2026-08-22 슬라이스 3 실행에서 요약이 *"max_pages를 올리면 이어서 수집합니다"* 라고 안내했으나 **거짓이었다** — 재실행이 아무것도 하지 못했다.
...
| 버전 | `schema_version` 정수. 챗 조정 명령이 **실변경을 만들며** 성공 시 +1(v2.12 문면 정정 — 전부 스킵된 성공 호출은 버전 의미상 무변경. 버전 = "생성에 영향을 주는 변경"의 추적), **AI 자동 등재 발생 실행마다 +1**, **수동 지시문 개정 시에도 +1**(v2.11 — schema_update 도구가 v2.12에서 구현돼 도구가 정공법이 되고 수동 편집은 예외 경로로 강등. 수동 경로만 씨앗·런타임 이중 갱신, **도구는 런타임만** 수정하고 "씨앗 미반영 — 클론 초기화 시 재적용 필요"를 고지), 셋 다 `updated_at` 갱신. 생성된 모든 KG JSON은 자기가 사용한 `schema_version`을 meta에 기록한다 → PRD "변경은 새 생성분부터(소급 없음)"의 추적 근거. ⚠ 알려진 별건: `registerNewTypes`가 +1은 하나 `updated_at`을 갱신하지 않아 이 규약과 어긋난다(2026-08-26 패널 실측 — 개선 큐 등재) |
...
```json
{
  ...
  "canonical_entities": [ "…(§2.3.2 v2.8 — 정본 엔티티 사전)" ],
  "property_overrides": [ "…(§2.3.2 v2.10 — 속성 승자 사전)" ]
}
```

- `origin: "seed" | "auto" | "manual"` — 시드/AI 자동 등재/챗 수동 조정의 출처 추적. `first_seen`은 자동 등재 유형의 최초 발생 stem(검수 시 역추적용).
- **사전 필드 2종 등재 (v2.10 정정 — 명세-실물 괴리 폐쇄)**: `canonical_entities`(v2.8)·`property_overrides`(v2.10)는 이 파일에 산다(위 예시에 v2.8 누락 상태였다). 두 사전은 **생성 프롬프트에 렌더되지 않고**(`renderSchema`는 유형 목록·`instructions_ko`만 렌더 — 실측) **`schema_version`도 올리지 않는다**(버전의 의미는 "생성에 영향을 주는 변경"의 추적 — 사전은 주입 단계 전용). 사전 수정 시 `updated_at`만 갱신하며, 재빌드는 원장 `build` 항목에 그 시각(`schemaUpdatedAt`)을 기록해 "같은 Reviewed/인데 결과가 다른" 경우의 재현성 근거로 삼는다. **시드(`schema.default.json`)의 `property_overrides`는 빈 배열이어야 한다** — 값 존재 검사가 있는 사전이라 사용자 고유 항목을 시드에 실으면 격리 e2e(시드 복사)가 미적용 경고로 오염된다(canonical_entities의 시드 탑재 관행과 의도적으로 다르다).
...
**시점 규율 2줄 (v2.11 신설 — 2026-08-26 Q7, 오너 B안)**: `instructions_ko`에 다음 2줄을 추가한다 — ①현재-상태 속성(주소·직책·규모 등)에는 자료가 현재 값으로 제시한 값만 쓰고, 특정 시점이 붙은 값(예: "설립 당시")은 그 속성에 쓰지 않는다 ②시점이 붙은 값은 버리지 않고 해당 사건(Event) 노드의 속성으로 기록한다(예: 설립 당시 본점 주소 → 설립 Event의 location·year). **근거**: p04에서 *"설립 (본점: 삼봉로 57)"* 의 과거 주소가 시점 한정 없이 `headquarters`(현재-상태 속성)에 들어가 병합 충돌의 상류 원인이 됐다 — 시점 자체는 같은 파일의 설립 Event(year·location)에 이미 보존되는 관행이 있어, 이 지시는 **기존 우수 산출 관행의 강제화**다. **미채택(2026-08-26 반박 패널)**: `<속성명>_as_of`·`former_` 접미 규약 — 키 단위 선착 병합에서 다른 파일의 현재 값과 **거짓 짝**을 이루고(예: p07의 현재 주소 + p04의 "설립 당시" 접미 속성), 전 코퍼스 시점 표기(숫자 연도 22건)와 다른 제3 규약을 신설하며, 파일 단독 추출에서 판정 불가한 과거화(former_) 추론을 강요한다. 지시문만의 개정도 `schema_version` **+1과 `updated_at` 갱신 의무**(§2.1.2 v2.11 보완 — meta 추적 근거 유지, Q6 선례 a33fc3e 준거이되 그 선례의 updated_at 누락은 이번에 수리).
...
- 그 이상(전각/반각, 부분 일치)은 **자동으로는** 하지 않는다 — 과병합 방지.
- **정본 엔티티 사전 (v2.8 신설 — 2026-08-23, §2.1.5의 "반복 검출 시 별칭 사전을 앞당기는 트리거" 발동)**: `data/schema.json`의 `canonical_entities`에 **사람이 명시한 항목만** 주입 단계에서 정본 (라벨, 이름)으로 정규화한다. 구현은 `pipeline/src/inject/canonicalize.js`(단일 구현), 적용 지점은 `mergeReviewed`의 **병합 키 계산 직전**이다.
  - 형식: `{ canonical: {label, name}, variants: [{label, name}], note }`. **라벨 범위**로 대조하므로 이름만 같은 다른 대상은 합쳐지지 않는다.
  - **연쇄 금지**: 정본이 다른 항목의 변형으로도 등록되면 재빌드를 중단한다 — 순서에 따라 결과가 갈려 멱등성이 깨진다.
  - **사전이 잘못되면 DB에 손대기 전에 멈춘다**(`reason: invalid_canonical`). 잘못된 사전은 멀쩡한 노드를 엉뚱하게 합치므로 조용히 넘어가면 안 된다.
  - **자동 추론은 여전히 하지 않는다** — 유사 이름 쌍 리포트가 후보만 올리고 합칠지는 사람이 정한다. 사전은 그 판단의 기록물이다.
  - **채택 근거 [실측] 2026-08-23**: `instructions_ko`에 이미 *"같은 대상은 반드시 같은 name으로 쓴다"* 가 있었는데도 `리디안솔루션`/`리디안솔루션(주)`로 갈렸고, 그 분리가 질문 1회에서 **거짓 부재 2건**을 만들었다. 지시문 경로는 실패가 증명됐고 정규화는 결정론적이다. "탄지로" vs "카마도 탄지로"는 병합되지 않으며, 이는 스키마 지시문의 "정식 명칭 통일"(2.1.5)로 상류에서 해결한다. 별칭 사전은 후순위.
...
- **속성 병합 규칙(속성 보존·보강 — v2.10 개정, Q10)**: 재빌드는 Reviewed/ 파일을 **파일명 오름차순(= 배치 타임스탬프 순 — 승인 시각의 권위는 있으나 내용 시점의 권위는 없다)**으로 처리한다. 없는 속성 키는 추가, 이미 있는 키에 다른 값이 오면 기본은 **선착 값 유지**(먼저 처리된 파일 우선). 결과: 같은 (schema.json, Reviewed/ 파일 집합 — **파일명 포함**)이면 몇 번 재빌드해도 같은 결과(멱등·결정적 — PRD S5 준수). v2.10에서 다음 2개 장치를 신설한다.
  - **속성 충돌 가시화 (v2.10 신설)**: 같은 병합 키·같은 속성 키에 **서로 다른 값**이 관측되면(**노드 속성만** — 관계 속성의 동형 결함은 개선 큐 **Q11** 별건) 충돌로 집계해 재빌드 요약에 보고한다. 기록 단위는 (노드, 속성)별 **고유 값 집합**(값마다 출처 파일 병기)이고 **(라벨, name_key, 속성 키)로 결정적 정렬** 후 상위 10 + 총계 — 파일 처리 순서에 무관하다. **값 동등성 = 정규화 후 심층 비교**: 문자열 NFC+trim, 배열은 요소별 정규화 후 **순서 보존** 비교(순서를 의미로 취급 — 순서 무시 비교는 목록형 정보를 지운다), 숫자·불리언은 **타입 포함** 일치(`2016 ≠ "2016"` — 타입 통합은 과병합 방지 원칙상 하지 않는다). 시스템 예약 속성은 수집에서 제외한다. 사전이 해결한 충돌은 미해결 목록에서 **빠진다** — 등재가 목록을 줄여야 품질 루프가 닫힌다.
  - **속성 승자 사전 `property_overrides` (v2.10 신설)**: `data/schema.json`의 `property_overrides`에 **사람이 명시한 항목만** 병합 후 적용한다. 형식 `{ "node": {"label","name"}, "property", "value", "note" }`. node ref는 정본 엔티티 사전(canonicalize)을 통과시키므로 변형 표기도 허용된다.
    - **선택 장치이지 발명 장치가 아니다**: 지정 값이 그 (노드, 속성)의 **관측값 집합(canonicalize 적용 후 병합 그래프 기준 — 선착에서 버려진 값 포함 전체)에 실제로 존재할 때만** 적용한다. 미관측(파일 갱신으로 사전이 낡음)·대상 노드/속성 부재 시 **적용하지 않고 재빌드는 계속하되**, 미적용 항목을 **전량**(잘림 없이) 사유·현재 관측값 목록과 함께 요약에 보고한다 — 조용한 no-op가 아니라 **소리 내는 no-op**다. *(반박 패널 개정: 초안의 "미관측·부재 시 전체 중단"은 반려·source_remove의 자동 재빌드를 지속 실패로 만들어 PRD 성공 기준 7과 충돌했다 — 중단에서 전량 보고로 강등. 미채택 대안: 하드 스톱 유지 — 비전문가 오너가 schema.json 수기 편집 전까지 DB 갱신 전체가 봉쇄됨.)*
    - **주입 전 중단은 사전 내부 모순만**: ①필수 필드 누락 ②같은 (병합 키, 속성) 중복 등재 — **canonicalize+name_key 적용 후 키로 검사**(변형 표기 이중 등재 우회 차단 — canonical_entities의 연쇄 금지와 동형) ③`property`가 시스템 예약 속성(`name`·`name_key`·`kgid`·`reviewed_files`·`input_files`) — name 교정은 canonical_entities 소관임을 메시지에 명시 ④`value` 타입이 문자열·숫자·불리언·동종 원시 배열 밖. 위반 시 `reason: 'invalid_property_overrides'` + `summary` 필드로 **DB에 손대기 전에** 중단한다(Q6 철학 — 잘못된 사전으로 조용히 진행하지 않는다).
    - **등재 단계화**: 1회성 충돌은 상류(원문·재검수) 수정이 제1 후보고, 사전 등재는 **반복 검출되거나 상류 수정이 비경제적일 때** 쓴다(§2.1.5 트리거 패턴과 동일 논리). 첫 항목(headquarters)은 오너 A안 결정(엔진 0회 즉시 수리 — DECISIONS 2026-08-26)에 따른 등재다.
    - **한계 3건(명시)**: ①충돌 감지는 **같은 속성 키**끼리만 본다 — 파일마다 키가 다르게 추출되면(예: `address` vs `headquarters`) 잡히지 않는다(속성 키 정규화는 별건 후보) ②미등재 충돌의 선착 폴백 승자는 파일명 순서 의존이다 — 재검수로 타임스탬프 접두가 바뀌면 승자가 바뀔 수 있고, 사전 등재가 그 비직관성을 흡수한다 ③`source_remove`(§4.3-9)는 사전을 건드리지 않는다 — 민감 자료 제거 시 사전에 남은 값·note 점검 안내를 도구 요약에 포함한다(슬라이스 8 구현 시 반영).
...
- **status**: `collected`(MD 저장 성공 — PRD의 "이미 수집된 페이지" 판정 기준) / `failed`(재실행 시 자동 재시도, `attempts`·`last_error` 갱신) / `blocked`(영구 차단 — 수집 시도 자체를 건너뜀. **v2.12: `blocked_at` 시각 필드 병기** — source_remove block 모드가 기입).
...
- **`last_reject_reason`(v2.7 신설 — 2026-08-23 슬라이스 6 구현)**: `review_reject`의 `reason` 인자를 500자로 잘라 기록한다(§4.3-7이 요구한 "원장 기록"의 필드명 확정). 같은 턴에 `last_attempt_at`도 갱신한다. **회차 N의 정본은 여전히 `reject_count`**이지만, 원장이 없거나 어긋난 경우 구현은 `max(reject_count, 기존 rejN 파일 수) + 1`을 써서 **기존 반려 이력을 덮어쓰지 않는다**(이력 보존 원칙 우선).
...
동작: 배치 시작 시각으로 타임스탬프 1회 고정, 등록 도메인 경계·BFS·robots.txt·요청 간격 준수(§1.5), 정규화 URL 기준 원장 스킵(1차 발견 키 + 2차 final_hash — §2.4.4), 실패분 자동 재시도(재실행 시). 반환 데이터: 신규 저장 파일명 목록(최대 10 표시 + 총수), 건너뜀 수, 실패 목록(URL·사유). 원장에 차단(`blocked`)된 URL은 건너뜀으로 보고. **상태 판정(v2.13)**: 실패>0이면 저장·스킵·중복 중 하나라도 있을 때 부분 성공, 없으면 실패. 실패 0인데 저장·스킵·중복이 전부 0이고 robots 차단만 있으면 **실패**로 보고하고 "사이트 정책 — 재시도 대상 아님"을 안내한다(재실행 권유 금지). force 재수집 파일은 저장 목록에 `(덮어씀)`으로 구분 표기(신규 아님). 실패 목록은 §4.1의 5건 상한을 따른다.
...
동작: 로컬 추출만(외부 전송 없음, §1.5). 저품질·빈 추출도 파일 생성 + 프론트매터에 품질 플래그(베스트에포트 정의). 반환: 성공/저품질/실패 목록. **폴더 배치(v2.13)**: 폴더면 그 안의 **파일만**(하위 폴더 제외) 지원 확장자 대상으로 이름순 처리하고, 비지원 파일은 제외 목록으로 보고한다. 한 파일의 실패는 배치를 중단하지 않고 실패 항목으로 남는다(실패 표시는 §4.1의 5건 상한). `path`는 **절대경로만** 수용 — MCP 서버의 cwd는 클라이언트 스폰 위치라 예측 불가(§1.10 계열). **차단(blocked) 스킵 안내**: "이미 추출됨"이 아니라 차단 상태로 보고하고, force가 통하지 않음(§2.4.4 blocked>force)과 해제 경로(source_remove `recollect_ok`)를 명시한다.
...
**v2.12 확정(3렌즈 반박 패널 반영 — 상세 근거는 DECISIONS 2026-08-26)**:
- **target 해석**: URL(`://` 포함)은 정규화 실패 시 안내 반환(도구 크래시 금지). `webKey` 미스 시 **`final_hash`·`final_url`·`source` 폴백 대조**(리다이렉트 최종 URL 입력 수용). 스킴리스 URL 추정 입력은 "https://를 붙여 다시" 안내. 파일명은 경로 문자 즉시 거부(1004행 계약)하되 **`.md`와 `.kg.json` 양쪽 수용**(`.kg.json`은 stem 환산 — review_list에서 복사하는 실수 예상 지점).
- **원장 전수 처리**: 같은 `file` 또는 같은 `final_hash`를 공유하는 **전 엔트리를 일괄** 삭제(recollect_ok)/차단(block) — 리다이렉트 쌍둥이 엔트리를 첫 매치 1건만 처리하면 재수집 영구 불능·차단 우회가 생긴다(패널 blocker 실증). 크롤러의 2차 dedupe(`findByFinalHash`)는 **blocked 엔트리도 대조**해 차단 문서의 신규 진입 URL 수집을 막는다.
- **키 복원 순서 고정**: 삭제 **전에** Input MD 프론트매터(`source_hash` = 원장 키 원문, §2.4.3)를 파싱 → block이면 그 키로 blocked 엔트리 확보 → 그 다음 삭제. 원장 미발견 + Input MD 부재 + block → "차단할 키 없음 — URL로 다시 시도" **명시 실패**(조용한 성공 금지).
- **부분 실패 계약**: ①읽기(키·형제 엔트리 확정) ②파일 전수 삭제 — removed[]·failed[] 수집 ③**전부 성공했을 때만** 원장 변경(saveLedger 원자적) ④Reviewed 실제 제거분 있으면 재빌드(실패 허용·노트) ⑤부분 실패는 실패 요약 + 전수 보고 + "다시 실행이 곧 복구"(실존분만 지우므로 멱등).
- **rej 회차 매칭**: 앵커 정규식 `^<stem>\.kg\.rej\d+\.json$`(§2.4.2 문면 일치 — startsWith는 타 stem 오폭 실증).
- **사전 점검 안내**(§2.3.2 v2.10 한계 ③): 사전 2종이 비어있지 않으면 "제거 자료에서 온 값·note 잔존 가능 — 점검" 1줄 동봉.
- **원장 필드 신설**: `blocked_at`(차단 시각 — §2.4.4 엔트리 필드에 추가).
...
**v2.12 확정(패널 반영)**: ①`set_instructions`는 **빈 배열·빈 문자열 요소 거부**(연산 기반 채택 사유였던 "통째 증발"을 이 인자가 재도입하는 자기모순 차단) + diff에 **교체 전 지시문 전문 동봉**(대화 로그가 사실상 백업 — 런타임 파일은 커밋 금지라 VCS 안전망 없음) ②remove 계열은 제거 항목의 **원본 레코드 전량**(origin·ko·desc·first_seen)을 diff에 반환(복원 재료)하고 경고 3종을 동봉 — 재산출 시 auto로 재등재되어 메타 소실 / 사전 2종이 그 라벨을 참조하면 정규화가 미등재 라벨을 계속 재생산(침묵 무효화) / Reviewed 잔존 라벨은 재빌드에서 침묵 통과 ③실변경 0건이면 버전 불변·"변경 없음" 보고 ④**어노테이션 `destructiveHint: true`** — SDK 1차 출처 문면("false = additive-only")상 remove·set은 additive가 아니고, 이전 값의 복구 불능 상실이 가능하다(승인 UX 부담 없음 — 오너 정책은 source_remove만 매번 확인) ⑤대상은 런타임 사본만 — "씨앗 미반영" 고지 동봉.
※ §4.3-10 `first_seen`은 **기록 코드가 없어 현재 전부 null**(스펙-실물 괴리 — 개선 큐 Q14). getSchema는 null을 정직 보고한다.
...
```json
{
  ...
  "unmatched": ["혈귀술"],
  "nodes": [ { "a": "n1", "name": "카마도 탄지로", "label": "Person", "hop": 0, "props": { "role": "주인공", "age": 15 } } ],
...
```
**노드 사실 속성 `props` (v2.9 개정 — 2026-08-23)**: 옛 형식은 `summary`(시스템 속성 제외 **문자열** 속성 중 **최장값 1개**, 160자)였다. 그 규칙이 **숫자 속성을 원천 배제**해, 회사 노드가 가진 속성 17개 중 챗에 도달한 것이 `description` 하나뿐이었고 `founded_year: 2016`이 사라졌다. 그 결과 답변이 *"설립 연도는 그래프에 없다"* 는 **거짓 부재**를 냈다 — 도구가 주지 않은 것을 챗이 말할 수는 없으므로 **챗 입장에서는 정직한 보고**였다. ⇒ `summary`를 **`props`(사실 속성 전부)** 로 교체한다. 구현은 `searchEngine.js`의 `nodeFacts` 단일 함수. 규칙: 시스템 속성(`name`·`name_key`·`kgid`·`reviewed_files`·`input_files`) 제외 · 문자열/숫자/불리언 전달 · 배열은 `, `로 연결 · **값당 160자 절단** · **노드당 24개 상한이며 넘으면 숨기지 않고 `propsOmitted`로 개수를 보고**한다 · 키는 정렬해 호출마다 같은 순서를 보장한다(Neo4j는 속성 순서를 보장하지 않는다). **경량화 계약(§6.3)과의 균형은 실측으로 정했다** — 실제 검색(1층 60노드)에서 노드 페이로드 **9.7KB**, 노드당 속성 수 중앙값이 1이라 대부분의 노드는 그대로다. **관계 속성은 아직 전달하지 않는다** — 검색 Cypher가 관계의 `properties()`를 조회조차 하지 않으며, 질의를 바꾸면 M4 골든 기준선이 흔들린다(개선 큐 **Q9**).
...
### 15) `kg_status` (인자 없음)
반환: 파이프라인(Input n / Generated 대기 n / Reviewed n / Rejected n / 보류 목록), DB(노드·관계 수, 마지막 buildId·재빌드 시각), 스키마(schema_version·최근 자동 등재 유형), 연결 상태(Neo4j 접속 가능?, 허브 가동?, 3D 뷰어 접속 수), 설정(기본 엔진).

**v2.12 구현 확정(패널 반영)**: ①수집은 도메인 `pipeline/src/status`가 소유(폴더·원장·스키마·엔진 + **DB count는 sessionFactory 주입**으로 도메인에 — 표면 파일에 Cypher를 반입하는 최초 선례를 만들지 않는다. D3 계쟁과 무관하게 pipeline은 합법 위치) ②DB count는 **RKEntity 스코프**(buildId counts와 표시 일관) ③**DB·허브 실패는 도구 실패가 아니라 "부분 성공"** — 각 섹션 상태와 `classifyNeo4jError` 힌트를 "다음 행동"에 싣는 것 자체가 이 도구의 정상 산출 ④허브 조회는 **GET `/api/health`**(로컬 허브 전용 — Vercel 배포본에는 없음. §5.1 라우트 등재)로 하며 응답은 `{ ok, connected }`(SSE 구독 수 — hub의 `connectedCount()`). 문제 발견 시 "다음 행동"에 복구 명령 안내(예: "Neo4j 접속 불가 — AuraDB 무료 인스턴스가 일시정지됐을 수 있습니다. console.neo4j.io에서 Resume을 확인하세요" — v2.1). ※ Neo4j 다운 시 각 도구의 동작: `kg_search` = 검색 실패 요약 + 하이라이트 미푸시, `kg_rebuild` = 잠금 해제 후 실패 요약, `review_reject` = ②단계 실패를 요약에 명시(§4.3-7).
...
| `/api/health` | GET | MCP 서버(kg_status) | 허브 가동·SSE 구독 수 조회 `{ ok, connected }` (v2.12 신설 — **로컬 허브 전용**, Vercel 배포본에 없음. 기존 라우트가 전부 푸시(POST·화면 영향) 또는 SSE(구독 자체가 카운트 오염)라 조회 전용 경로가 없었다) | — (본문 없음) |

기존 `MAX_BODY_BYTES` 100KB 단일 상수는 **라우트별 상한**으로 확장한다(위 표 — v2 명시). `/api/health`는 본문 수신 블록을 타지 않는 조기 반환 라우트다(상한 맵 미등재 — undefined 비교 함정 회피, 패널 실측).
