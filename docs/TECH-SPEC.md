...
**한계 — 이 스냅샷이 증명하지 않는 것**: 이것은 앱이 **어떤 집합을 어느 상태로 판정했는가**를 증명하지, **그 판정이 실제로 그 색으로 칠해졌는가**를 증명하지 않는다. 스냅샷과 `Graph3D.jsx`의 색 적용(`nodeHighlightState` → `HIGHLIGHT_COLOR`/`brightenColor`)은 같은 `resolved`에서 갈라져 나온 **평행 경로**이므로, 색 적용 단계의 결함은 스냅샷에 나타나지 않는다. 그 구간은 여전히 육안 또는 별도 수단(E2E)이 필요하다 — 판정 ④의 사정거리를 "집합 판정까지"로 이해할 것.
