...
> 상태: **v2.2 — v2.1(AuraDB 전환·스캐폴딩 패널 반영) 후 총감사(패널 8인) 개정: 검증 계획 6(호출률·부정 대조) 신설, §5.5 404 비치명 처리, §6.4 Aura RTT 재산정, citation 문구 정직화, 과잉 인용 경고, 잠금 감지, README 목차 ⑮~⑰, 부록 A 3건 해소, 신형 콘솔 자격증명 정정 (상세는 부록 C·DECISIONS.md)**  · **v2.3(2026-08-22 유지보수 M2·M3): §6.2.3 질문 원문 앵커 복원 / §6.3 T2 점수 하한 / §6.4 상한 3행 / §4.3-13 인용 밀도 안내 정직화 / §1.14 가정 7행 신설** · **v2.4(2026-08-22): §7.9 하이라이트 상태 진단 스냅샷(`window.__bibliomind`) 신설 — 읽기 전용, 렌더링 무변경** · **v2.5(2026-08-22 슬라이스 3): §1.5 스킵된 페이지의 링크 되살리기 신설 — 재실행 이어받기 공백 해소(네트워크 요청 0회 유지)** · **v2.6(2026-08-22 슬라이스 1.5): §1.4 엔진 플래그 세트 실측 확정 — --max-turns 부재로 제거, --output-schema 구조적 비호환으로 미채택** · **v2.7(2026-08-23 슬라이스 6): §2.4.4 `last_reject_reason` 신설** · **v2.8(2026-08-23 Q6): §2.3.2 정본 엔티티 사전(`canonical_entities`)** · **v2.9(2026-08-23 Q8): §6.3 `nodeFacts`(속성 전달)** · **v2.10(2026-08-26 Q10): §2.3.2 속성 충돌 가시화 + 속성 승자 사전(`property_overrides`) · §2.1.2 사전 필드 등재 · §4.3-8 반환·안내 분기 (4렌즈 반박 패널 반영 — v2.7~v2.9는 인라인만 있고 이 목록에 누락돼 있어 함께 등재)** · **v2.11(2026-08-26 Q7): §2.1.5 시점 규율 지시문 2줄(Event 단일화 — _as_of 접미 미채택) · §2.1.2 수동 지시문 개정 경로 보완(3렌즈 반박 패널 반영)**| 최종 수정: 2026-08-26
...
| 버전 | `schema_version` 정수. 챗 조정 명령 성공 시 +1, **AI 자동 등재 발생 실행마다 +1**, **수동 지시문 개정 시에도 +1**(v2.11 보완 — schema_update 도구 미구현 동안의 수동 파일 편집 경로. 씨앗·런타임 이중 갱신), 셋 다 `updated_at` 갱신. 생성된 모든 KG JSON은 자기가 사용한 `schema_version`을 meta에 기록한다 → PRD "변경은 새 생성분부터(소급 없음)"의 추적 근거. ⚠ 알려진 별건: `registerNewTypes`가 +1은 하나 `updated_at`을 갱신하지 않아 이 규약과 어긋난다(2026-08-26 패널 실측 — 개선 큐 등재) |
