...
- **최우선 규칙**은 `~/.claude/CLAUDE.md`(사용자 전역)에 있다 — *"요청·지시·질의의 핵심과 맥락을 분석하여 관련 탑티어 전문가를 투입하라."* **오너 상시 규칙 8조**는 워크스페이스 루트 `CLAUDE.md` 말미가 정본이며 그 제1조가 위 전역 규칙의 시행 세칙이다(2026-08-22 채택). 상위에서 열지 않아 로드되지 않았다면 두 절을 먼저 읽는다 — 근거·유예 목록은 `docs/OPERATING-RULES-7.md`.
