...
- **노드·관계 라벨**: 성능 보호를 위해 라벨은 화면 표시분이 **노드 120·관계 80 이하**일 때만 자동 표시됩니다. 큰 그래프에서 보려면 그래프 우상단 **"시각화 스타일"** 드롭다운에서 — **"노드 이름 표시"**(모든 노드가 이름 텍스트로 표시) 또는 **"관계 라벨 표시"**(모든 관계 유형 표시)를 선택하세요. 드롭다운이 단일 선택이라 **두 라벨을 동시에 켤 수는 없습니다**(교차 전환으로 사용).
