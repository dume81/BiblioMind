...
### 한눈에 — 자료 뭉치 하나를 끝까지 그래프로 만드는 순서

웹사이트 하나(하위 페이지 전부)든 PDF 폴더든, 같은 5단계 루프입니다. 모든 단계가 중단·재개에 안전하므로 며칠에 나눠 해도 이어집니다.

| 단계 | 챗에 입력할 문장 | **끝났다는 신호** |
|---|---|---|
| 1. 수집 | 웹: "collect_web으로 https://예시.com 을 상한 30으로 수집해줘" / 문서: "collect_docs로 C:\\자료폴더 추출해줘" | 웹: **같은 문장을 반복해 "저장 0건·건너뜀만" 나오면 사이트 전체 완료** / 문서: 결과 목록이 폴더 전체(저장·스킵·실패)를 보고 |
| 2. 생성 | "kg_generate로 대기 자료 2건 생성해줘" — 같은 문장 반복 | **"남은 대기 0건"** (건당 2~4분 — 언제 끊어도 다음에 이어짐) |
| 3. 검수 | "review_list 보여줘" → "N번 화면에 보여줘" → "승인" / "반려: 사유" | 검수 대기 0건 |
| 4. 반영 | "kg_rebuild 실행해" — 한 번이면 됨 | 요약의 노드·관계 수 확인 |
| 5. 의미 검수 | 자료에 대해 몇 가지 질문 → 3D에서 근거 확인 | 이상한 경로가 없으면 완성. 있으면 그 자료를 반려(3단계) → 재생성 검수 → 재빌드 — 이 반복이 품질 루프입니다 |

### 1) 수집

- **웹**: "collect_web으로 https://example.com 수집해줘" — 같은 도메인 안(하위 도메인 포함)을 기본 10페이지까지 수집합니다(상한은 "상한 30으로"처럼 조절). robots.txt를 지키고, **같은 명령 재실행 = 기수집 스킵·이어서 수집**입니다. **사이트 전체를 원하면 "저장 0건"이 나올 때까지 같은 문장을 반복**하세요 — 그게 더 갈 페이지가 없다는 완료 신호입니다.
- **문서(PDF·이미지)**: "collect_docs로 C:\자료\보고서.pdf 추출해줘" 또는 **폴더째** "collect_docs로 C:\자료폴더 추출해줘"(안의 지원 파일 전부를 이름순 처리 — 경로는 절대경로로). 지원 확장자: PDF와 이미지(png·jpg·jpeg·webp·bmp·tif·tiff). 추출은 **로컬에서만** 일어나 본문이 이 기계를 벗어나지 않고, 이미지 OCR 언어팩(한+영)만 최초 1회 온라인 다운로드 후 캐시됩니다.
- **같은 파일 재투입은 자동 스킵**됩니다 — 파일명이 바뀌어도 내용이 같으면 알아봅니다(재추출하려면 "force로"). 
- **한계(베스트에포트)**: 텍스트 레이어 없는 스캔본·저화질 이미지는 품질 등급(ok/low/empty)이 낮게 표시됩니다. **PDF가 empty로 나오면** 스캔본일 가능성이 큽니다 — 해당 페이지를 이미지(PNG 등)로 저장해 다시 넣으면 OCR 경로로 처리됩니다. low 등급은 생성 결과가 빈약할 수 있으니 검수에서 유심히 보세요.
...
- **반려 사유는 재생성 지시로 전달됩니다** — "반려: 'AI Tech'와 'AI Tech 사업부'를 하나의 이름으로 통일해 재생성"처럼 구체적으로 쓰면 다음 산출물이 그 지시를 반영합니다. 재빌드 요약이 알려주는 "유사 이름 쌍"을 이 방식으로 정리하면 그래프가 깔끔해집니다.
...
- **강조된 경로가 원문과 다르면** 그 자료를 검수 절 방식으로 반려하고 재빌드하세요 — 질문이 곧 최종 검수이며, 이 반복으로 그래프 품질이 올라갑니다.
...
### 3D 화면 보는 법 — 통합 그래프와 라벨

- **전체 그래프 불러오기**: 3D 앱의 Neo4j 탭은 기본 한도 **노드 1,000·관계 2,000**(서버 최대치)으로 전체를 불러옵니다(2026-08-28 상향). 이전 기본값(300/600)에서는 큰 그래프의 관계가 잘려 **실제로는 연결된 노드가 화면에서 고립된 것처럼 보이는** 현상이 있었습니다 — Neo4j 콘솔과 모습이 다르면 먼저 한도를 의심하세요. 그래프가 이 한도를 넘게 커지면 같은 절단이 다시 생깁니다(그때는 한도 재결정 필요).
- **노드·관계 라벨**: 성능 보호를 위해 라벨은 화면 표시분이 **노드 120·관계 80 이하**일 때만 자동 표시됩니다. 큰 그래프에서 보려면 그래프 우상단 **"시각화 스타일"** 드롭다운에서 — **"텍스트 노드"**(모든 노드가 이름 텍스트로 표시) 또는 **"관계 라벨 항상 표시"**(모든 관계 유형 표시)를 선택하세요. 드롭다운이 단일 선택이라 **두 라벨을 동시에 켤 수는 없습니다**(교차 전환으로 사용).
- 스타일과 무관하게 **마우스 호버 = 속성 툴팁, 클릭 = 그 노드·관계 라벨 표시**는 항상 동작합니다. 화면이 버벅이면 드롭다운을 "기본"으로 되돌리거나 "대규모 성능 모드"를 선택하면 즉시 회복됩니다.
